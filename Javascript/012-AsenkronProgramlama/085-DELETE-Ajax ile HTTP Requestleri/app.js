class Obje{
    ...
    DELETE(url, callback){
        ...
        this.xhr.onload = function(){
            if(this.status==200){
                callback(null, this.responseText + " Silme islemi basariyla gerceklesti") // Burada responseText bos bir obje gostereceginden bunu da yanina yazmak daha makul geldi.
            }else{
                callback("Delete request : Delete islemi bir hata ile karsilasti.", null);
            }
        }
        this.xhr.send();
    }
...
albums.DELETE("https://jsonplaceholder.typicode.com/albums/11", function(Error, Deleted){
    if(Error==null){
        console.log(Deleted);
    }else{
        console.log(Error);
